...
class RtvSpider(scrapy.Spider):
    ...
    def build_repair(self):
        repair_file_path = self.radio_station + '_' + self.saving_code + '_repair.xls'
        if os.path.exists(repair_file_path):
            ...
            if len(self.urls) == 0:
                ...
                path = self.radio_station + '_' + self.saving_code + '.xls'
    ...
    def parse_error(self, response):
        if response.value.response.status == 408 or response.value.response.status == 500 \
            ...
            self.repair_xls.save(self.radio_station + '_repair.xls')
            self.repair_row += 1
    ...
    def save_list(self):
        ...
        for track in self.all_tracks:
            ...
            if self.row == 30001:
                ...
                self.row = 1
        if not self.repair_opt:
            self.list_xls.save(self.radio_station + '_' + self.saving_code + '.xls')
        else:
            self.list_xls.save(self.radio_station + '_' + self.saving_code + '_repaired.xls')